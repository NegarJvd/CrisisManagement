        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 overflow-x-auto dark:text-gray-100">
                    <form method="post" action="{{ route('cnc-supply.update', $cnc->id) }}" class="mt-6 space-y-6">
                        @csrf
                        @method('put')
                            ...
                            <div class="basis-1/2 mr-2">
                                ...
                                <x-select multiple id="machines" name="machines[]" class="mt-1 block w-full" required autofocus autocomplete="machines">
                                    @foreach($machines as $machine)
                                        <option value="{{$machine->id}}" @if(in_array($machine->id, old('machines', $cnc->machines()->pluck('id')->toArray()))) selected @endif>{{$machine->name}}</option>
                                    @endforeach
                                </x-select>
